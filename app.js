/* rAAA!nking — calificando películas caga'o de miedo */
(() => {
  "use strict";

  const CONFIG = Object.assign({ tmdbKey: "", language: "es-MX" }, window.RAAANKING_CONFIG || {});
  ...
  const KEYS = {
    ratings: "raaanking.ratings.v1",
    draft: "raaanking.draft.v1",
    tmdbKey: "raaanking.tmdbKey",
    sort: "raaanking.sort"
  };
  ...
  function exportRatings() {
    ...
    const payload = { app: "rAAA!nking", version: 1, exportedAt: new Date().toISOString(), ratings };
    const blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `raaanking-${new Date().toISOString().slice(0, 10)}.json`;
  ...
  async function importRatings(file) {
    try {
      ...
      setFooterMsg("Ese archivo no parece un respaldo de rAAA!nking.", "error");
    }
  }
